...
export async function DELETE(
  ...
  try {
    ...
    console.log(`[DELETE] Deleting practice set ${id}`);
    console.log(`[DELETE] Auth header:`, authHeader ? 'Present' : 'Not present');
    console.log(`[DELETE] Cookie header:`, cookieHeader ? 'Present' : 'Not present');
    console.log(`[DELETE] Token from cookie:`, token ? 'Present' : 'Not present');
    ...
    if (authHeader) {
      headers['Authorization'] = authHeader;
      console.log('[DELETE] Using Authorization header from request');
    } else if (token) {
      headers['Authorization'] = `Bearer ${token}`;
      console.log('[DELETE] Using token from cookie for Authorization header');
    } else {
      console.log('[DELETE] No authorization token found');
    }
    
    console.log(`[DELETE] Sending request to: ${backendUrl}/practice/sets/${id}`);
    ...
    console.error(`[DELETE] Error deleting practice set ${id}:`, error);
    return NextResponse.json(
      { message: 'Failed to delete practice set', error: String(error) },
      { status: 500 }
    );
  }
}
